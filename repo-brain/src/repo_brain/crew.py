...
# Canned lessons used only while brain.py is still a stub (NotImplementedError).
# Warm demo task ("/orders") gets these; cold ("/items") gets none — so Person 2
# can rehearse both paths before Person 1 lands.
_FAKE_WARM_LESSONS: list[dict[str, Any]] = [
    {
        "type": "convention",
        "rule": (
            "Error responses use {'error': {'code': SCREAMING_SNAKE, 'message': human text}} "
            "via app.errors.api_error(status, code, message); never raise a bare HTTPException."
        ),
        "evidence": "Reviewer rejected tutorial-style HTTPException(status_code=404, detail='not found').",
        "source_task": "add a /items endpoint",
        "score": 0.93,
        "hit_count": 1,
    },
    {
        "type": "convention",
        "rule": (
            "Route handlers are named handle_<resource>_<verb> "
            "(e.g. handle_items_list, handle_items_get), snake_case, one resource per module in app/routes/."
        ),
        "evidence": "Reviewer rejected list_items / get_item naming.",
        "source_task": "add a /items endpoint",
        "score": 0.91,
        "hit_count": 1,
    },
    {
        "type": "convention",
        "rule": (
            "Every route declares response_model= with a Pydantic model in the same module named "
            "<Resource><Verb>Response (e.g. ItemsListResponse, ItemsGetResponse)."
        ),
        "evidence": "Reviewer flagged missing response_model on GET /items.",
        "source_task": "add a /items endpoint",
        "score": 0.89,
        "hit_count": 1,
    },
    {
        "type": "convention",
        "rule": (
            "Tests live in one file per resource; functions named test_<resource>__<behavior> "
            "(double underscore), using the shared client fixture."
        ),
        "evidence": "Reviewer rejected test_list_items; wanted test_items__list_returns_all.",
        "source_task": "add a /items endpoint",
        "score": 0.87,
        "hit_count": 1,
    },
...
def _chat_model() -> BaseChatModel:
    ...
    raise RuntimeError(
        "No LLM key set — fill GOOGLE_API_KEY (Gemini), ANTHROPIC_API_KEY, or OPENAI_API_KEY"
    )
...
def _complete(system: str, user: str) -> str:
    ...
    for attempt in range(4):
        try:
            ...
            return _text(msg)
        except Exception as exc:  # noqa: BLE001 — Gemini free-tier 429s during rehearsal
            err = str(exc)
...
def _search_lessons(task: str) -> list[dict[str, Any]]:
    """Real brain when Person 1 has landed; canned fallback until then."""
    try:
        return list(brain.search_lessons(task) or [])
    except NotImplementedError:
        if "order" in task.lower():
            return list(_FAKE_WARM_LESSONS)
        return []
    except Exception as exc:  # noqa: BLE001 — keep the crew moving during parallel build
        print(f"[crew] brain.search_lessons failed ({exc!r}); using fake brain")
        if "order" in task.lower():
            return list(_FAKE_WARM_LESSONS)
        return []
...
def reviewer(state: CrewState) -> dict[str, Any]:
    raw = _complete(
        system=(
            ...
            "- If the code already follows all conventions, approve immediately — even on first review.\n"
            "- On a REJECT pass, report at most TWO violations, highest priority first.\n"
            "  Priority: (1) error envelope, (2) handler naming, (3) response models, (4) test naming.\n"
